...
def render_time_series_forecast(df_featured: pd.DataFrame, model, model_name: str):
    ...
    if st.button("Generate 14-Day Forecast", type="primary", use_container_width=True):
        with st.spinner("Generating forecast..."):
            ...
            for _ in range(13):
                next_val = forecast_values[-1] * np.random.normal(1.0, 0.05)
...
with tab5:
    ...
    if model_to_manage in st.session_state.challenger_models:
        ...
        # We need to sanitize column names for the challenger data just like in retraining
        X_test_challenger = X_test.copy()
        sanitized_cols = [re.sub(r'[^A-Za-z0-9_]+', '', col) for col in X_test_challenger.columns]
        X_test_challenger.columns = sanitized_cols

        X_test_champ = prepare_data_for_model(X_test, champion_model.feature_names_in_)
        X_test_chall = prepare_data_for_model(X_test_challenger, challenger_model.feature_names_in_)
...
with tab6:
    ...
    - **Key Drivers**: The most influential factors are overall hospital capacity, regional population, and demographics.
    - **Actionable Insights**: The dashboard transforms complex model outputs into clear intelligence for decision-makers.
    """)

    st.subheader("Strategic Recommendations for Stakeholders")
    st.markdown("#### For Hospital Administrators:")
    st.markdown("""
    - **Resource Allocation**: Use the single-region forecast to anticipate bed shortages and manage resources.
    - **Capacity Planning**: Analyze projections to inform long-term strategic decisions about expanding ICU capacity.
    - **Peer Benchmarking**: Compare your region's forecast with others to identify best practices.
    """)

    st.markdown("#### For Public Health Officials:")
    st.markdown("""
    - **Identify Hotspots**: Use the bulk forecast to identify regions at high risk and target them for support.
    - **Policy Making**: Leverage feature insights to inform targeted public health policies.
    - **Emergency Preparedness**: Use the tool as an early-warning system to prepare for demand surges.
    """)

    st.markdown("#### For Data Science & MLOps Teams:")
    st.markdown("""
    - Regularly use the **MLOps & Monitoring** tab to retrain and evaluate challenger models.
    - Monitor the **Data Drift** section to know when a model needs retraining to prevent degradation.
    """)

    st.subheader("Understanding Model Explainability (SHAP & LIME)")
    st.markdown("""
    - **Why did the model say that?** The SHAP and LIME plots build trust by answering why a forecast was made.
    - **SHAP**: Breaks down a prediction to show the precise contribution of each feature.
    - **LIME**: Provides an intuitive, high-level summary of the most important factors for a single prediction.
    """)
